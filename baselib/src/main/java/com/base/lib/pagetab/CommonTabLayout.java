...
public class CommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    private static final String TAG ="CommonTabLayout";
...
//            tv_tab_title.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
    ...
    private void calcOffset() {
        ...
        TextView currentTabText = currentTabView.findViewById(R.id.tv_tab_title);

        final View lastTabView = mTabsContainer.getChildAt(this.mLastTab);
        TextView lastTabText = lastTabView.findViewById(R.id.tv_tab_title);

        Log.d(TAG,"the currentTab is "+mCurrentTab+";the lastTab is "+mLastTab);
        ...
        if(mIndicatorWidthEqualTitle){
            mCurrentP.left = currentTabText.getLeft();
            mCurrentP.right = currentTabText.getRight();

            mLastP.left = lastTabText.getLeft();
            mLastP.right = lastTabText.getRight();
        }
        Log.d(TAG, "mLastP--->" + mLastP.left + "&" + mLastP.right);
        Log.d(TAG, "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    private void calcIndicatorRect() {
        ...
        TextView tabText = currentTabView.findViewById(R.id.tv_tab_title);

        float left = currentTabView.getLeft();
        float right = currentTabView.getRight();

        if(mIndicatorWidthEqualTitle){//与标题等宽
            left = tabText.getX();
            right = tabText.getX()+tabText.getWidth();
        }
